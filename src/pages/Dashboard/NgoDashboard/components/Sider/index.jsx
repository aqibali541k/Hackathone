    ...
    >
      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-slate-800/60 shrink-0">
        <Link to="/" className={`flex items-center gap-3 transition-opacity ${!isSiderOpen && "opacity-0 invisible w-0"}`}>
          <div className="bg-emerald-500 rounded-lg p-1.5 flex items-center justify-center">
        ...
        <button
          ...
          className={`w-10 h-10 flex items-center justify-center rounded-xl hover:bg-white/10 text-slate-400 hover:text-white transition-colors ${!isSiderOpen ? "mx-auto" : ""}`}
        >
          ...
          return (
            <Link
              ...
              className={`relative flex items-center gap-4 px-3 py-3 rounded-xl transition-all duration-200 group
                ${isActive 
                  ? "bg-emerald-600/10 text-emerald-400 font-medium before:absolute before:left-0 before:top-2 before:bottom-2 before:w-1 before:bg-emerald-500 before:rounded-r-full" 
                  : "hover:bg-slate-800/50 hover:text-slate-100"
                }`}
            >
              <div className={`text-xl transition-colors shrink-0 ${isActive ? "text-emerald-500" : "text-slate-400 group-hover:text-slate-200"}`}>
                {item.icon}
              </div>
